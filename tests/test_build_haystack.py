...
from haystack_test import HaystackConfig, build_haystack


class TestBuildHaystack:
    def test_returns_text_and_pairs(self, seeded_random):
        seeded_random(42)
        result = build_haystack(HaystackConfig(haystack_num=10))
    ...
    def test_correct_number_of_pairs(self, n, seeded_random):
        seeded_random(42)
        _, pairs = build_haystack(HaystackConfig(haystack_num=n))
        assert len(pairs) == n

    @pytest.mark.parametrize("n", [1, 10, 100])
    def test_correct_number_of_lines(self, n, seeded_random):
        seeded_random(42)
        text, _ = build_haystack(HaystackConfig(haystack_num=n))
    ...
    def test_keys_are_uppercase_strings(self, seeded_random):
        seeded_random(42)
        _, pairs = build_haystack(HaystackConfig(haystack_num=50))
        for key, _ in pairs:
            assert key.isupper()
            assert key.isalpha()
    ...
    def test_values_in_range(self, val_min, val_max, seeded_random):
        seeded_random(42)
        _, pairs = build_haystack(HaystackConfig(haystack_num=50, val_min=val_min, val_max=val_max))
        for _, val in pairs:
    ...
    def test_default_key_length(self, seeded_random):
        seeded_random(42)
        _, pairs = build_haystack(HaystackConfig(haystack_num=10))
        for key, _ in pairs:
            assert len(key) == 8

    def test_custom_key_length(self, seeded_random):
        seeded_random(42)
        _, pairs = build_haystack(HaystackConfig(haystack_num=10, key_len=6))
        for key, _ in pairs:
            assert len(key) == 6

    def test_all_keys_unique(self, seeded_random):
        seeded_random(42)
        _, pairs = build_haystack(HaystackConfig(haystack_num=100))
        keys = [k for k, _ in pairs]
        assert len(keys) == len(set(keys))

    def test_text_matches_pairs(self, seeded_random):
        seeded_random(42)
        text, pairs = build_haystack(HaystackConfig(haystack_num=20))
        for line in text.strip().split("\n"):
